...
//				else
			...
			case DataStructs.STATUS.STREAMING:
				...
				if (reset) {
					reset = false;
					yawReset = sensorData.yaw;
				}
				#if !USE_QUATERNION
				if (!ignoreTracking && trinusCamera != null)
					trinusCamera.transform.localEulerAngles = new Vector3 (sensorData.pitch, (sensorData.yaw - yawReset) * settingsInstance.yawScale, -sensorData.roll);
				#else
